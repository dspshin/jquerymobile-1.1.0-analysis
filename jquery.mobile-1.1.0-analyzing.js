...
// 20110720 code review start (dsp)-----------------------------------------------------------
/*!
...
$.widget = function( name, base, prototype ) {
	var namespace = name.split( "." )[ 0 ],	// "mobile.button"과 같이 들어올 경우, namespace "mobile"가 됨.
		fullName;
	name = name.split( "." )[ 1 ]; // "mobile.button"의 경우 name은 button. 예를 들어, "a.b.c"가 name으로 들어올 경우, c는 무시.
	fullName = namespace + "-" + name; // fullName = "mobile-button"
	...
	// ":" 뒤에 mobile-button과같은 selector가 추가됨. 
	// $.expr에 대해 추가 조사 필요 #TBD
	...
	var basePrototype = new base(); // 동작에 대해 추가 조사 필요 #
	// new base()를 하면 shallow copy를 하는 듯, 그래서 deepcopy를 하기 위해 아래 줄이 필요한가부다. 체크 필요!

	basePrototype.options = $.extend( true, {}, basePrototype.options ); // deepcopy
	$[ namespace ][ name ].prototype = $.extend( true, basePrototype, {
		namespace: namespace,
		widgetName: name,
		widgetEventPrefix: $[ namespace ][ name ].prototype.widgetEventPrefix || name,
		widgetBaseClass: fullName
	}, prototype ); // prototype은 user가 define한 prop+method들을 포함하여 확장.

	$.widget.bridge( name, $[ namespace ][ name ] ); // bridge 사용.
};

// bridge : public/private method 등록 및 구분.
//          $.data setting
$.widget.bridge = function( name, object ) {
	$.fn[ name ] = function( options ) {
		var isMethodCall = typeof options === "string",
			args = Array.prototype.slice.call( arguments, 1 ), // arguements.slice(1)과 동일한데 이게 뭔소리?
			returnValue = this;

		// allow multiple hashes to be passed on init
		// options가 string이 아니고, object인 경우, options에 args를 추가로 확장.
		options = !isMethodCall && args.length ?
			$.extend.apply( null, [ true, options ].concat(args) ) :
			options;

		// prevent calls to internal methods
		// private 함수가 들어올 경우에는 실행하지 않고 this를 리턴.
		...
		if ( isMethodCall ) {
			this.each(function() {
				...
				var methodValue = instance[ options ].apply( instance, args ); // 실제 method call
			...
			this.each(function() {
				...
				if ( instance ) {
					...
					$.data( this, name, new object( options, this ) ); // setter
				}
			});
		}
...
$.Widget.prototype = {
	...
	_createWidget: function( options, element ) {
		...
		console.log("Widget _createWidget called");
	...
	_getCreateOptions: function() {
		console.log("Widget _getCreateOptions called");
		var options = {};
		if ( $.metadata ) {
			options = $.metadata.get( element )[ this.widgetName ]; // $.metadata ?? ##TBD
		}
	...
	option: function( key, value ) {
		console.log("Widget option called");
		var options = key;
	...
	_setOptions: function( options ) {
		console.log("Widget _setOptions called");
		var self = this;
	...
	enable: function() {
		console.log("Widget enable called");
		return this._setOption( "disabled", false );
	},
	disable: function() {
		console.log("Widget disable called");
		return this._setOption( "disabled", true );
	},

	// trigger 추가 분석 필요 ##TBD
	_trigger: function( type, event, data ) {
		console.log("Widget _trigger called :"+type);
